...
class Goods extends Common
{
	...
	public function add(){
        ...
        if (cookie('imgupload2')) {
            $cookie_arr2 = unserialize(cookie('imgupload2'));
            foreach ($cookie_arr2 as $key => $value) {
                $url_pre2 = DS.'layui2'.DS.'public';
                $url2 = str_replace($url_pre2,'.',$value);
                if (file_exists($url2)) {
                    unlink($url2);
                }
            }
		...
		if (session('goods_thumb')){
            ...
            $url_pre2 = DS.'layui2'.DS.'public';
            $url2 = str_replace($url_pre2,'.',session('goods_thumb'));
	...
	public function uploadthumb(){
	    ...
	    if($info){
	        $address = DS.'layui2'.DS.'public' . DS . 'uploads'.DS.$info->getSaveName();
	...
	public function addhanddle(){
		//添加商品表单提交处理
		$post = request()->post();
        $post['goods_time'] =date('Y-m-d H:i:s');//时间
        $post['time'] =time();//时间
...
//		}
        ...
        $imgupload2 = $_SESSION['imgupload2'];
		...
		if ($goods_add_result) {
            ...
            foreach ($imgupload2 as $key => $value) {
                if($value!='0'){
                    $goods->png()->save(['url'=>$value]);
                }
            }
	...
	public function goodslist($goods_pid=''){
		...
		$page_class = new \app\admin\controller\Page($goods_totle,8);
	...
	public function upd($goods_id=''){
        ...
        $img_select2 = db('png')->where('goods_id','eq',$goods_id)->select();
        ...
        if (isset($_SESSION['imgupload2'])) {
            foreach ($_SESSION['imgupload2'] as $key => $value) {
                $url_pre2 = DS.'layui2'.DS.'public';
                $url2 = str_replace($url_pre2,'.',$value);
                if (file_exists($url2)) {
                    unlink($url2);
        ...
        foreach ($img_select2 as $key => $value) {
            $_SESSION['imgupload2'][] = '1';
            $_SESSION['old2'][] = $value['url'];
        }

		//获取旧的信息
		$this->assign('img_select',$img_select);
        $this->assign('img_select2',$img_select2);
		return view();
	}
...
//            $url_pre2 = DS.'layui2'.DS.'public';
//            $url2 = str_replace($url_pre2,'.',$img_url);
...
//            }
		...
		if ($goods_add_result!==false) {
            ...
            foreach ($imgupload2 as $key => $value) {
                if($value == '-1'){
                    //旧图片删除
                    db('png')->where('url','eq',$_SESSION['old2'][$key])->delete();
                    $url_pre2 = DS.'layui2'.DS.'public';
                    $url2 = str_replace($url_pre2,'.',$_SESSION['old2'][$key]);
                    if (file_exists($url2)) {
                        unlink($url2);
                    }
                }
	...
	public function del($goods_id=''){
		...
		//删除商品的缩略图
            ...
            $goods_png = $goods_get->png()->select();

			$goods_img_toArray = $goods_img->toArray();
            $goods_png_toArray = $goods_png->toArray();
            ...
            foreach ($goods_png as $key => $value) {
                $url_pre2 = DS.'layui2'.DS.'public';
                $url2 = str_replace($url_pre2,'.',$value['url']);
                if (file_exists($url2)) {
                    unlink($url2);
                }
            }
    ...
    public function imgupload(){
        ...
        if($info){
            $address = DS.'layui2'.DS.'public' . DS . 'uploads'.DS.'img'.DS.$info->getSaveName();
    ...
    public function imgupload2(){
        //商品细节图上传的方法
        $file = request()->file('goods_img2');
        $info = $file->move(ROOT_PATH . 'public' . DS . 'uploads'.DS.'png');
        if($info){
            $address = DS.'layui2'.DS.'public' . DS . 'uploads'.DS.'png'.DS.$info->getSaveName();
            $_SESSION['imgupload2'][] = $address;
            $session_str = serialize($_SESSION['imgupload2']);
            cookie('imgupload2',$session_str,3600);
            return $address;
        }else{
            echo $file->getError();
        }
    ...
    public function imgcancle2(){
        if (request()->isAjax()) {
            $post = request()->post();

            $img_index = $post['index'];
            $img_address = $_SESSION['imgupload2'][$img_index];
            if($img_address==1){
                $_SESSION['imgupload2'][$img_index] = '-1';
            }
            else{
                $_SESSION['imgupload2'][$img_index]='0';
            }
            $url_pre2 = DS.'layui2'.DS.'public';
            $url2 = str_replace($url_pre2,'.',$img_address);
            if (file_exists($url2)) {
                unlink($url2);
            }
        }
