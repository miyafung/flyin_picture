...
class Property extends \think\Controller
{
    ...
    public function del($property_id=''){
        //删除管理员的方法
        $property_find = db('property')->find($property_id);
        if (empty($property_find)) {
            $this->redirect('property/propertylist');
        }
        $property_del_result = db('property')->delete($property_id);
        if ($property_del_result) {
            $this->success('删除成功','property/propertylist');
        }
        else{
            $this->error('删除失败','property/propertylist');
        }
    }
